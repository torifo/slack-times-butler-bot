...
SLACK_APP_TOKEN=...
JAPAN_AI_API_KEY=...
JAPAN_AI_BASE_URL=https://api.japan-ai.co.jp
JAPAN_AI_MODEL=gpt-4o
JAPAN_AI_ARTIFACT_IDS=...
JAPAN_AI_CHAT_ENDPOINT=/chat/v2
JAPAN_AI_TEMPERATURE=0.1
SOURCE_CHANNEL=...
POST_TARGET_CHANNEL=...
```
...
* `JAPAN_AI_API_KEY`: JAPAN AI API の認証用キー

### 18.6 JAPAN AI API の実装前提

このプロジェクトでは、JAPAN AI API を次の前提で使う。

* リクエストは `POST`
* レスポンスは `JSON`
* 文字コードは UTF-8
* Base URL は `https://api.japan-ai.co.jp`
* 現在の実装は `POST /chat/v2` を使い、`stream: false` で非ストリーミング応答を受ける
* 認証は `Authorization: Bearer <API_KEY>` を使う

RAG を使う場合は、JAPAN AI プラットフォームで取得した `artifactId` を `JAPAN_AI_ARTIFACT_IDS` に設定する。
